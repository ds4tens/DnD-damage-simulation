...
class BaseCharacter {
    ...
    constructor(level: number, characterClass: BaseClass, weapon: Weapon, weaponPrimaryStat: StatsTypes = 'strength', statBlock: StatBlock = defaultStatBlock, critThershold: number = 20) {
        this._level = level
    ...
    _makeHitCheck(actionModifier: TAttackModifier): {isCrit: boolean, isHit: boolean, hitChance: number} {
        ...
        const rawHitChance = actionModifier.hasAdvantage ? Math.max(new Dice(20).rollWithNormalDistribution(), new Dice(20).rollWithNormalDistribution()) :
        actionModifier.hasDisadvantage ? Math.min(new Dice(20).rollWithNormalDistribution(), new Dice(20).rollWithNormalDistribution()) 
    ...
    _makeDamageCalculation(actionModifier: TAttackModifier, isCrit: boolean): number {
        const damageBonusWithActionModifier = actionModifier.hasDamageModifier ? actionModifier.damageModifierFunction() : 0;
        const damageBonusWithStatModifier = this.calculateDamageBonus();
        if (isCrit) {
            const critDamage = this._weapon.damage.reduce((acc, dice) => acc + dice.rollWithNormalDistribution() + dice.rollWithNormalDistribution(), 0);
            return critDamage + damageBonusWithStatModifier + damageBonusWithActionModifier;
        }
        return this._weapon.damage.reduce((acc, dice) => acc + dice.rollWithNormalDistribution(), 0) + damageBonusWithStatModifier + damageBonusWithActionModifier;
    }
    ...
    // } 
    attack(): number {
        this._characterClass.makeBonusAction().forEach(action => action());
